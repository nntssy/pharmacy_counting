...
def collectInfoByGroup(filename,isFloat):    
    """
    ...
    if isFloat:
        ...
        for drug in sorted(drugs.keys(),key=lambda x:(-drugs[x][0],x.strip('"'))):
            output_list.append((-int(drugs[drug][0]),str(drug.strip('"')),int(len(drugs[drug][1])),str(drug)))
    os.remove(os.path.join(os.getcwd(), filename+".json"))
    return output_list
    
def collectGroups(filenames,isFloat):
    """
    ...
    for name in filenames:
        list_of_lists.append(collectInfoByGroup(name,isFloat))
...
def writeInfo(sorted_list,fout,isFloat):
    """
...
if __name__ == "__main__":    
    ...
    with open(input_file,'r') as fin:
        filenames=divideInput(fin,isFloat)
    list_of_lists=collectGroups(filenames,isFloat)
    sorted_list=collectTotalInfo(list_of_lists)
    with open(output_file,'w') as fout:
        writeInfo(sorted_list,fout,isFloat)